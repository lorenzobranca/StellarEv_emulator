...
TIME_DIR  = "/export/data/vgiusepp/StellarEv_emulator/preprocessing_new_log15/"
...
# =========================
    ...
    for i in range(int(OUTPUT_DIM)):
        ...
        for j, idx in enumerate(idxs):
            ...
            print("mean and varaince:", m, s)
            print("scaled pred min and max:", np.amin(y_model), np.amax(y_model))
            print("scaled true min and max:", np.amin(y_true), np.amax(y_true))
            print("descaled pred min max", np.amin(1.5**(y_model*s + m)), np.amax(1.5**(y_model*s + m)))
            print("descaled true min max", np.amin(1.5**(y_true*s + m)), np.amax(1.5**(y_true*s + m)))
            # Difference plot
